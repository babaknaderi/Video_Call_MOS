...
class MlvqNet(nn.Module):
    def __init__(
            ...
            pad=100,
            features = [
            'integer_motion', 'integer_motion2','integer_adm2', 'integer_adm_scale0', 'integer_adm_scale1',
            'integer_adm_scale2', 'integer_adm_scale3', 'integer_vif_scale0', 'integer_vif_scale1', 'integer_vif_scale2', 'integer_vif_scale3', 
            ]
            ):
        super().__init__()
...
def get_features(df_features, features):
    ...
    def _get_frame_freeze_feature(x_ref_frames):
        ...
        for i, x_cur in enumerate(x_ref_frames[1:], start=1):
            ...
            x_prev = x_cur
        return x_freezes
    if 'ref_frames' in df_features.columns:
...
class MLVideoQualityModel():
    def __init__(self, checkpoint="vcm/mlvqa_model_weights.pt"):
        self.dev = torch.device("cuda") if torch.cuda.is_available() else torch.device("cpu")
        print(f"Running on {self.dev}")
        self.model = VcmNet()
        self.model.to(self.dev)
    ...
    def __call__(self, deg_video, ref_video, results_dir, tmp_dir, verbosity):
        os.makedirs(results_dir, exist_ok=True)
        if verbosity:
            print(f"Computing MLVQ for video {deg_video} with reference {ref_video}")
        ...
        # save results to CSV
        df_results['mlvqa_mos'] = mos_frame
        results_csv = os.path.join(results_dir, os.path.basename(deg_video).split('.')[0]+'.csv')
        df_results.to_csv(results_csv, index=False)

        if verbosity:
            print(f"MLVQ computation done. Results saved at {results_csv}")            
        return mos, results_csv